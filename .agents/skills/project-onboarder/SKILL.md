...
- **If the tool succeeds (Phase 2 - Context Gathering):**
  1. Ask the user: *"What is the Asana Team ID for this client?"* (Suggest they look at the first number in their Asana URL).
  2. Ask the user: *"What is the Asana Project ID for this repository?"* (The second number in the URL).
  3. Ask the user for the target `GCP Project ID`.
  4. Read `.agents/rules/asana-context.template.md`, replace the `{{VARIABLES}}` with the provided IDs, and save the new file as `.agents/rules/asana-context.md`.
  5. Tell the user: *"Workspace initialized! You can now use `/asana-start` to begin vibe coding."*
