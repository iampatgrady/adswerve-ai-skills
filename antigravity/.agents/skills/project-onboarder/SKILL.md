...
- **If the tool is missing or errors (Phase 1):**
  ...
  3. Read `~/.gemini/antigravity/mcp_config.json` (create it with an empty `{"mcpServers": {}}` object if it doesn't exist).
  ...
  5. Save the file.
  6. **STOP.** Tell the user: *"I have configured your MCP plumbing in `~/.gemini/antigravity/mcp_config.json`. Please restart your IDE, authorize the Asana popup, and run `@project-onboarder` again to finish setup!"*
